...
test('workbook round trip: private references, numeric quantities, full summaries and unknown/zero values', () => {
  ...
  for (const entry of report.statements) {
    ...
    assert.deepEqual(book.SheetNames, ['Summary', 'Daily Details', 'Office References']);
    ...
    const daily = XLSX.utils.sheet_to_json(book.Sheets['Daily Details'], { range: 10 });
    const dailySubtotals = daily.filter(row => row['Quality details'] === 'Daily subtotal');
    assert.equal(dailySubtotals.length, statement.dailyTotals.length);
    assert.equal(dailySubtotals.reduce((sum, row) => sum + row.Cones, 0), statement.monthlyTotals.cones);
    assert.equal(dailySubtotals.reduce((sum, row) => sum + Math.round(row['Net kg'] * 1000), 0), statement.monthlyTotals.netGrams);
    const summary = XLSX.utils.sheet_to_json(book.Sheets.Summary, { range: 10 });
    assert.equal(summary.at(-1).Cones, statement.monthlyTotals.cones);
    assert.equal(summary.at(-1)['Net kg'], statement.monthlyTotals.netKg);
    assert.ok(summary.every(row => typeof row.Cones === 'number' && typeof row['Net kg'] === 'number'));
    ...
    if (entry.worker.id === 'worker/0') assert.match(summary.at(-1).Completeness, /Incomplete/);
