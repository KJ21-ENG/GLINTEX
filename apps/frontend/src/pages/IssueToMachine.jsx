...
export function IssueToMachine() {
  ...
  const handleSendSummary = async () => {
    ...
    try {
      const stage = process === 'holo' ? 'holo' : process === 'coning' ? 'coning' : 'cutter';
      const result = await sendSummaryNotification(stage, 'issue', summaryDateFrom, summaryDateTo, summaryShifts);
      if (result.ok) {
  ...
  const handleDownloadSummary = async () => {
    ...
    try {
      const stage = process === 'holo' ? 'holo' : process === 'coning' ? 'coning' : 'cutter';
      await downloadSummaryPdf(stage, 'issue', summaryDateFrom, summaryDateTo, summaryShifts);
