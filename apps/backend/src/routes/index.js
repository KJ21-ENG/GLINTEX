...
router.get('/api/reports/barcode-history/:barcode', async (req, res) => {
    try {
        ...
        async function traceFromInbound(item, history) {
            ...
            const cutterIssue = await prisma.issueToCutterMachine.findFirst({
                where: { pieceIds: { contains: item.id } },
                include: { machine: true, operator: true, cut: true },
            });
        ...
        async function traceFromConingReceive(recv, history) {
            const issue = await prisma.issueToConingMachine.findUnique({
...
// Mark items as steamed
router.post('/api/boiler/steam', async (req, res) => {
    try {
        const actor = getActor(req);
        const { barcodes } = req.body; // Array of scanned barcodes (box or piece)
        ...
        const result = await prisma.$transaction(async (tx) => {
            // 1. Resolve all items to be steamed
            const itemsToSteamIds = new Set();
            ...
            for (const code of barcodes) {
                ...
                const box = await tx.box.findFirst({
                    where: { name: { equals: rawCode, mode: 'insensitive' } }
                });

                if (box) {
                    // If box, find all undispatched, unsteamed Holo rows in this box
                    // We only stream stuff currently in the box.
                    const rows = await tx.receiveFromHoloMachineRow.findMany({
                        where: {
                            boxId: box.id,
                            isDeleted: false,
                            steamedAt: null, // Only steam unsteamed items
                            dispatchedWeight: 0 // Only steam undispatched items (optional check)
                        }
                    });
                    for (const r of rows) itemsToSteamIds.add(r.id);
                    resolvedDetails.push({ code: rawCode, type: 'box', count: rows.length });
                } else {
                    // Check if it's a specific Row (Piece)
                    // Try exact barcode match first
                    let row = await tx.receiveFromHoloMachineRow.findUnique({
                        where: { barcode: rawCode }
                    });

                    if (!row) {
                        // Try legacy barcode resolution if needed, or other formats
                        const legacy = await resolveLegacyReceiveRow(rawCode);
                        if (legacy?.found && legacy.stage === 'holo') {
                            row = legacy.row;
                        }
                    }
                    ...
                    if (row) {
                        ...
                        // If already steamed, we can skip or re-steam?
                        // Requirement says "mark all of them as steamed". 
                        // If already steamed, maybe just ignore or update timestamp?
                        // Let's assume we update it to the new batch if re-scanned.
                        itemsToSteamIds.add(row.id);
                        resolvedDetails.push({ code: rawCode, type: 'piece', count: 1 });
                    } else {
                        // Log warning or error? For now just skip unknown barcodes
                        console.warn(`Boiler scan: Unknown barcode ${rawCode}`);
                    }
                }
            }

            if (itemsToSteamIds.size === 0) {
                return { count: 0, message: 'No valid unsteamed items found' };
            }

            // 2. Create BoilerBatch
            const batch = await tx.boilerBatch.create({
                data: {
            ...
            // 3. Update Rows
            const updateResult = await tx.receiveFromHoloMachineRow.updateMany({
                where: { id: { in: Array.from(itemsToSteamIds) } },
                data: {
                    steamedAt: new Date(),
                    steamedByUserId: actor?.userId,
                    boilerBatchId: batch.id
                }
            });
        ...
        await logCrudWithActor(req, {
            entityType: 'boilerBatch',
            entityId: result.batchId,
            action: 'create',
            payload: { count: result.count, barcodes }
        });

        res.json({ ok: true, ...result });

    } catch (err) {
        console.error('Boiler steam failed', err);
        res.status(500).json({ error: err.message || 'Steaming failed' });
    }
});
