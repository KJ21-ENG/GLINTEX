> Presentation revision (2026-09-07): The worker-facing preview, PDF and first Excel sheet now use one calendar row for every day of the selected month, dash placeholders for no recorded work, worker-specific yarn weight columns, daily total cones/kg and a monthly totals row. Yarn totals combine items, quality variants and machines; office references preserve the original detailed records. This replaces the summary/ledger presentation requirements below, which remain as historical acceptance evidence. PDF is one page per worker: A4 for up to five yarns, A3 landscape for larger sets (wider for more than 22 yarns). Unusually long labels fail explicitly rather than clipping. Dates use dd/mm/yyyy. The compact PDF calendar reserves space for a yarn-wise total-weight summary; the preview and Excel also include this summary. All-workers ZIP packaging is retained. Local review command: `node apps/backend/src/services/workerMonthlyReport/__tests__/localPreview.mjs`; loopback report-only preview uses read-only `glintex_dev` and omits the unused missing `isOpeningStock` field in the older local schema only.
